...
from utils import generate_payroll_pdf, format_currency, format_date
...
def list_payrolls():
    ...
    return render_template('payroll/list.html', payrolls=payrolls, employees=employees, 
                         selected_employee=employee_id, selected_status=status)
...
def create_payroll():
    ...
    if request.method == 'POST':
        try:
            ...
            payroll = Payroll(
                ...
                other_discounts=float(request.form.get('other_discounts', 0)),
                notes=request.form.get('notes', '')
            )
            
            # Calcular pago de horas extras
            if payroll.overtime_hours > 0:
                hourly_rate = employee.base_salary / 30 / 8  # Asumiendo 30 días y 8 horas diarias
                from config import config
                payroll.overtime_pay = payroll.overtime_hours * hourly_rate * config.OVERTIME_MULTIPLIER
            
            # Calcular totales
            from config import config
            payroll.calculate_totals(config)
    ...
    return render_template('payroll/form.html', 
                         employees=employees, 
                         payroll=None,
...
def edit_payroll(payroll_id):
    ...
    if request.method == 'POST':
        try:
            ...
            payroll.bonuses = float(request.form.get('bonuses', 0))
            payroll.other_discounts = float(request.form.get('other_discounts', 0))
    ...
    return render_template('payroll/form.html', payroll=payroll, employees=None)
